...
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y git build-essential cmake python3 curl
WORKDIR /build
...
RUN git apply 0001-Fix-build-on-debian-11.patch
