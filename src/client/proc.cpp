...
constexpr DawnProcTable DefaultProcTable() {
    DawnProcTable procs = {};
    procs.createInstance = webGPUDDCreateInstance;
    return procs;
}

static DawnProcTable procs = DefaultProcTable();

void webGPUDDSetProcs(const DawnProcTable* procs_) {
    procs = *procs_;
}
...
WGPUBool wgpuAdapterGetFormatCapabilities(WGPUAdapter adapter, WGPUTextureFormat format, WGPUFormatCapabilities * capabilities) {
...
void wgpuSharedBufferMemoryGetProperties(WGPUSharedBufferMemory sharedBufferMemory, WGPUSharedBufferMemoryProperties * properties) {
    procs.sharedBufferMemoryGetProperties(sharedBufferMemory, properties);
}
...
void wgpuSharedBufferMemoryReference(WGPUSharedBufferMemory sharedBufferMemory) {
    procs.sharedBufferMemoryReference(sharedBufferMemory);
}
